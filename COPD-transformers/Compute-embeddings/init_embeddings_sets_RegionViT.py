...
def read_files(file_path, num_slices, model_id, project_name):
    ...
    if num_slices == 9:
        test_list = pd.read_csv(file_path+f'/common_testing_{model_name}_{project_name}.csv', sep=",", usecols=diagnosis,header=0,low_memory=False)
    elif num_slices == 20:
        test_list = pd.read_csv(file_path+f'/common_testing_{model_name}_{project_name}_all.csv', sep=",", usecols=diagnosis,header=0,low_memory=False)
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(args.path, f"COPDGene-embeddings/{args.project_name}{args.model}-features_sets-{args.slices}")
    for set_i in [1,2]:
        if set_i == 1:
            img_dir = os.path.join(args.path, f"{args.project_name}-imgs")
        elif set_i == 2:
            img_dir = os.path.join(args.path, f"{args.project_name}2-imgs")
