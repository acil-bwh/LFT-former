...
def npy_to_one(model_id,slices,project_name,how,which,kernel):
    input_dir = f"{project_name}-features_kernels-{slices}"
    output_dir = input_dir

    test = f"{how}_test_labels_{which}_k{kernel}.npy"
    ...
    output_name = f"{how}_test_labels_{which}_k{kernel}_{model_id}.npy"
        ...
        for how in ["concat","mean","weighted","max","min","var","std","mix","pca"]:
            for which in ["E","T","C","ET","CT","EC","ETC"]:
                ...
                for model_id in modality_values:
                    print(f"\n--- One-label extraction for {model_id} using {slices} slices for {how} and {which} kernel {kernel} ---\n")
                    npy_to_one(model_id,slices,project_name,how,which,kernel)
