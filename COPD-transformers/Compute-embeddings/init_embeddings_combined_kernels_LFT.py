...
def combination(how,num_slices,which,project_name,csv_match,kernel):

    # --- Load Validation Features and Labels ---

    test_1 = safe_load(f"{project_name}1-features_kernels-{num_slices}/test_features_k{kernel}.npy")
    test_2 = safe_load(f"{project_name}2-features_kernels-{num_slices}/test_features_k{kernel}.npy")
    test_3 = safe_load(f"{project_name}3-features_kernels-{num_slices}/test_features_k{kernel}.npy")

    labtest_1 = safe_load(f"{project_name}1-features_kernels-{num_slices}/test_labels_k{kernel}.npy")
    labtest_2 = safe_load(f"{project_name}2-features_kernels-{num_slices}/test_labels_k{kernel}.npy")
    labtest_3 = safe_load(f"{project_name}3-features_kernels-{num_slices}/test_labels_k{kernel}.npy")
    ...
    os.makedirs(f"{project_name}-features_kernels-{num_slices}", exist_ok=True)

    np.save(f"{project_name}-features_kernels-{num_slices}/{how}_test_features_{which}_k{kernel}.npy", combined_featuresV)
    np.save(f"{project_name}-features_kernels-{num_slices}/{how}_test_labels_{which}_k{kernel}.npy", combined_labelsV)

    print(f"Combined V features using {how} for task(s) {which} saved!\n",flush=True)
    return

csv_match = f"{project_name}files/matching_indices_kernels.csv"
for kernel in [1,2,3,4,5,6]:
    for num_slices in [9,20]:
        print(f"\n--- Combining for {num_slices} slices --- kernel {kernel} \n")
        for how in ["concat","mean","weighted","max","min","var","std","mix","pca"]:
            for which in ["E","T","C","ET","CT","EC","ETC"]:
                combination(how,num_slices,which,project_name,csv_match,kernel)

print("Multi-label and feature extraction finished for 9 and 20 slices using multiple embeddings and fusion methods using all kernels")