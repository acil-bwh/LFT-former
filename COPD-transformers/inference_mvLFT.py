...
from Utils.inference.predictions_mvLFT import Predictor_mvLFT
import argparse
...
for num_slices in [20]:
    print(f"\nPredicting mvLFT {output_type} using {input_type} with {how}, views with {how_view}, adding {vars_add} with {wrapping_mode} ---- {num_slices}")
    predicting = Predictor_mvLFT(main_path,
                                num_slices,
                                cuda_id,
                                how,
                                how_view,
