...
from Utils.inference.predictions_msLFT import Predictor_msLFT
import argparse
...
for num_slices in [20]:
    print(f"\nPredicting msLFT {output_type} using {input_type} with {how} adding {vars_add} with {wrapping_mode} ---- {num_slices}")
    predicting = Predictor_msLFT(main_path,
                                num_slices,
                                cuda_id,
                                how,
                                project_name,
