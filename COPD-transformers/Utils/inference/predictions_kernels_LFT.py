...
# ---------------- MODEL PREDICTOR ----------------
def PredictorKernels_LFT(main_path,
                            num_slices,
                            cuda_id,
                            how,
                            project_name,
    ...
    features_dir = os.path.join(main_path, f"{project_name}-features_kernels")
    csv_path = os.path.join(main_path, f"{project_name}-files",f"df_common_val_{model_name0}.csv")

    if kernel in [1,2,3,4,5,6]:
        valid_features_path = os.path.join(f"{features_dir}-{num_slices}", f"{how}_test_features_{input_type}_k{kernel}.npy")
        valid_labels_path = os.path.join(f"{features_dir}-{num_slices}", f"{how}_test_labels_{input_type}_k{kernel}_{IT+1}.npy")
    else:
        raise ValueError("kernel must be in [1...6]")
    ...
    # ----------- SAVE RESULTS -----------
    base_csv = os.path.join(save_path, f"model_LFT_{output_type}_{fusion_type}-{how}_{input_type}_{vars_add0}_{wrapping_mode}_k{kernel}.csv")
    probs_csv = os.path.join(save_path, f"probs_LFT_{output_type}_{fusion_type}-{how}_{input_type}_{vars_add0}_{wrapping_mode}_k{kernel}.csv")
    raw_probs_csv =  os.path.join(save_path, f"rawprobs_LFT_{output_type}_{fusion_type}-{how}_{input_type}_{vars_add0}_{wrapping_mode}_k{kernel}.csv")
    df = pd.DataFrame({
