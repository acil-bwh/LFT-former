...
for model_id, task in tasks.items():
    ...
    for num_slices in [20]:
        ...
        csv_file = os.path.join(origin_path, f"probs_mvLFT_{output_type}_{fusion_type}-{how}_{how_view}_{input_type}_{vars_add}_{wrapping_mode}.csv")
        ...
        results_df.to_csv(os.path.join(csv_path, f"classAUC_mvLFT_{output_type}_{fusion_type}-{how}_{how_view}_{input_type}_{vars_add}_{wrapping_mode}.csv"), index=False)
        ...
        plt.grid(False)
        plt.tight_layout()
        plt.savefig(os.path.join(save_path, f"rocAUC_mvLFT_{output_type}_{fusion_type}-{how}_{how_view}_{input_type}_{vars_add}_{wrapping_mode}.png"), dpi=300)
        plt.close()
