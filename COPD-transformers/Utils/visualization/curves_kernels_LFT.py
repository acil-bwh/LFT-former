...
for kernel in [1,2,3,4,5,6]:
    for model_id, task in tasks.items():
        ...
        for num_slices in [20]:
            ...
            results_df.to_csv(os.path.join(csv_path, f"classAUC_LFT_{output_type}_{fusion_type}-{how}_{input_type}_{vars_add}_{wrapping_mode}.csv"), index=False)
            ...
            plt.grid(False)
            plt.tight_layout()
            plt.savefig(os.path.join(save_path, f"rocAUC_LFT_{output_type}_{fusion_type}-{how}_{input_type}_{vars_add}_{wrapping_mode}_k{kernel}.png"), dpi=300)
            plt.close()