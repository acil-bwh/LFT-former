...
def Confusion_mvLFT(main_path,
    ...
    csv_path = os.path.join(
        read_path, f"model_mvLFT_{output_type}_{fusion_type}-{how}_{how_view}_{input_type}_{vars_add}_{wrapping_mode}.csv")
    ...
    print(f"\n···· {model_name.upper()} MODEL {how} view: {how_view} and {vars_add} with {wrapping_mode} ({num_slices}-slices) ····")
    ...
    plt.title(f"MODEL {how} view: {how_view} and {vars_add} with {wrapping_mode}", fontsize=16)
    plt.xticks(rotation=45)
    plt.tight_layout()

    save_file = os.path.join(save_path, f"confusion_mvLFT_{output_type}_{fusion_type}-{how}_{how_view}_{input_type}_{vars_add}_{wrapping_mode}.png")
    plt.savefig(save_file, dpi=300, bbox_inches='tight')
    plt.close()

    print(f"Saved confusion matrix → {save_file}\n")

def OneOff_mvLFT(main_path,
                    num_slices,
                    how,
                    how_view,
                    project_name,
    ...
    # ---------------- READ CSV ----------------
    csv_path = os.path.join(read_path, f"model_mvLFT_{output_type}_{fusion_type}-{how}_{how_view}_{input_type}_{vars_add}_{wrapping_mode}.csv")
    if not os.path.exists(csv_path):
        raise FileNotFoundError(f"Results CSV not found: {csv_path}")
    ...
    print(f"\n···· {model_name.upper()} MODEL one-off {how} view: {how_view} and {vars_add} with {wrapping_mode} ({num_slices}-slices) ····")
    ...
    plt.xticks(rotation=45)
    plt.title(f"MODEL {how} view: {how_view} and {vars_add} with {wrapping_mode} one-off", fontsize=16)
    plt.tight_layout()

    save_file = os.path.join(save_path, f"confusion_oneoff_mvLFT_{output_type}_{fusion_type}-{how}_{how_view}_{input_type}_{vars_add}_{wrapping_mode}.png")
    plt.savefig(save_file, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Saved confusion matrix {save_file}\n")