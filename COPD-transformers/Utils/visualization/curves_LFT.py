...
if output_type == "EMPH":
    ...
    tasks = {1: {"prob_cols": ["probs_class0","probs_class1","probs_class2","probs_class3"], 
        "labels": {0:'Low',1:'Mild',2:'Moderate',3:'High'}, "add": 0}}
    add = 0
...
for model_id, task in tasks.items():
    ...
    for num_slices in [20]:
        ...
        if project_name == "ECLIPSE":
            csv_file = os.path.join(origin_path, f"probs_LFT_{output_type}_{fusion_type}-{how}_{input_type}_{vars_add}_{wrapping_mode}{axis_suffix}_{L}_{v}.csv")
            pred_file = os.path.join(origin_path, f"model_LFT_{output_type}_{fusion_type}-{how}_{input_type}_{vars_add}_{wrapping_mode}{axis_suffix}_{L}_{v}.csv")
        else:
            csv_file = os.path.join(origin_path, f"probs_LFT_{output_type}_{fusion_type}-{how}_{input_type}_{vars_add}_{wrapping_mode}{axis_suffix}.csv")
            pred_file = os.path.join(origin_path, f"model_LFT_{output_type}_{fusion_type}-{how}_{input_type}_{vars_add}_{wrapping_mode}{axis_suffix}.csv")
        ...
        if project_name == "ECLIPSE":
            results_df.to_csv(os.path.join(csv_path, f"classAUC_LFT_{output_type}_{fusion_type}-{how}_{input_type}_{vars_add}_{wrapping_mode}{axis_suffix}_{L}_{v}.csv"), index=False)
        else:
            results_df.to_csv(os.path.join(csv_path, f"classAUC_LFT_{output_type}_{fusion_type}-{how}_{input_type}_{vars_add}_{wrapping_mode}{axis_suffix}.csv"), index=False)
        ...
        plt.tight_layout()
        if project_name == "ECLIPSE":
            plt.savefig(os.path.join(save_path, f"rocAUC_LFT_{output_type}_{fusion_type}-{how}_{input_type}_{vars_add}_{wrapping_mode}{axis_suffix}_{L}_{v}.png"), dpi=300)
        else:
            plt.savefig(os.path.join(save_path, f"rocAUC_LFT_{output_type}_{fusion_type}-{how}_{input_type}_{vars_add}_{wrapping_mode}{axis_suffix}.png"), dpi=300)
        plt.close()
