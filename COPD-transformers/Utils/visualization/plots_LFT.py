...
def Confusion_LFT(main_path,
    ...
    read_path, f"model_LFT_{vars_add}_{wrapping_mode}.csv")
    ...
    print(f"\n···· {model_name.upper()} MODEL {vars_add}-{wrapping_mode} ····")
    ...
    plt.title(f"LFT {vars_add}-{wrapping_mode}", fontsize=16)
    plt.xticks(rotation=45)
    plt.tight_layout()

    save_file = os.path.join(save_path, f"confusion_LFT_{vars_add}_{wrapping_mode}.png")
...
def OneOff_LFT(main_path,
    ...
    print(f"\n···· {model_name.upper()} MODEL {vars_add}-{wrapping_mode} One-off ····")
    ...
    plt.xticks(rotation=45)
    plt.title(f"LFT {vars_add}-{wrapping_mode} one-off", fontsize=16)
    plt.tight_layout()

    save_file = os.path.join(save_path, f"confusion_oneoff_LFT_{vars_add}_{wrapping_mode}.png")
    plt.savefig(save_file, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Saved confusion matrix {save_file}\n")