...
def Confusion_msLFT(main_path,
    ...
    csv_path = os.path.join(
        read_path, f"model_msLFT_{output_type}_{fusion_type}-{how}_{input_type}_{vars_add}_{wrapping_mode}.csv")
    ...
    print(f"\n···· {model_name.upper()} MODEL {how} {wrapping_mode} ({num_slices}-slices) ····")
    ...
    plt.title(f"{model_name} msLFT {how} {wrapping_mode} ({num_slices}-slices)", fontsize=16)
    plt.xticks(rotation=45)
    plt.tight_layout()

    save_file = os.path.join(save_path, f"confusion_msLFT_{output_type}_{fusion_type}-{how}_{input_type}_{vars_add}_{wrapping_mode}.png")
    plt.savefig(save_file, dpi=300, bbox_inches='tight')
    plt.close()

    print(f"Saved confusion matrix → {save_file}\n")

def OneOff_msLFT(main_path,
                    num_slices,
                    how,
                    project_name,
                    output_type,
    ...
    # ---------------- READ CSV ----------------
    csv_path = os.path.join(read_path, f"model_msLFT_{output_type}_{fusion_type}-{how}_{input_type}_{vars_add}_{wrapping_mode}.csv")
    if not os.path.exists(csv_path):
        raise FileNotFoundError(f"Results CSV not found: {csv_path}")
    ...
    print(f"\n···· {model_name.upper()} MODEL {how} {wrapping_mode} {input_type} for {output_type} task ({num_slices}-slices) One-off ····")
    ...
    plt.xticks(rotation=45)
    plt.title(f"{model_name} msLFT {how} {wrapping_mode} ({num_slices}-slices) One-off", fontsize=16)
    plt.tight_layout()

    save_file = os.path.join(save_path, f"confusion_oneoff_msLFT_{output_type}_{fusion_type}-{how}_{input_type}_{vars_add}_{wrapping_mode}.png")
    plt.savefig(save_file, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Saved confusion matrix {save_file}\n")