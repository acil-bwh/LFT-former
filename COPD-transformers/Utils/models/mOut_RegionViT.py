...
class mOut_RegionViT(nn.Module):
    def __init__(
        self,
        *,
        dim=(64, 128, 256, 512),
        ...
        for ind, dim_layer, num_layers in zip(range(4), dim, depth):
            not_first = ind != 0
            need_downsample = not_first
            need_peg = not_first and use_peg

            self.layers.append(nn.ModuleList([
                Downsample(current_dim, dim_layer) if need_downsample else nn.Identity(),
                PEG(dim_layer) if need_peg else nn.Identity(),
                R2LTransformer(dim_layer, depth=num_layers, window_size=window_size,
                               attn_dropout=attn_dropout, ff_dropout=ff_dropout)
            ]))

            current_dim = dim_layer

        # Shared feature reduction (global pooling)
        self.feature_reduce = nn.Sequential(
            Reduce('b c h w -> b c', 'mean'),
            nn.LayerNorm(last_dim)
        )

        # Dynamically create output heads per task
        self.heads = nn.ModuleDict()
        for task, values in dict_out.items():
            if isinstance(values, list):
                # classification task
                self.heads[f"task{task}"] = nn.Linear(last_dim, len(values))
            else:
                # regression task
                self.heads[f"task{task}"] = nn.Linear(last_dim, 1)

    def forward(self, x, return_feats=False, return_all_stages=False):
        *_, h, w = x.shape
        assert divisible_by(h, self.region_patch_size) and divisible_by(w, self.region_patch_size), \
            'height and width must be divisible by region patch size'
        assert divisible_by(h, self.local_patch_size) and divisible_by(w, self.local_patch_size), \
            'height and width must be divisible by local patch size'

        # Encode
        local_tokens = self.local_encoder(x)
        region_tokens = self.region_encoder(x)

        stage_feats = []  # collect features per stage

        # Pass through RegionViT layers
        for down, peg, transformer in self.layers:
            ...
            if return_all_stages:
                # store region tokens for this stage
                stage_feats.append(region_tokens)

        # Global pooling
        feats = self.feature_reduce(region_tokens)

        # prediction heads
        out = {task: head(feats) for task, head in self.heads.items()}

        # Decide what to return
        if return_feats and return_all_stages:
            return out, feats, stage_feats
        elif return_feats:
            return out, feats
        else:
            return out
