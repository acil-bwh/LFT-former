...
from Utils.inference.predictions_sets_LFT import PredictorSets_LFT
import argparse
...
for i in range(0,2):
    ...
    for num_slices in [20]:
        print(f"\nPredicting LFT {output_type} using {input_type} with {how} adding {vars_add} with {wrapping_mode} for set {set_num} ---- {num_slices}")
        predicting = PredictorSets_LFT(main_path,
                            num_slices,
                            cuda_id,
                            how,
                            project_name,
