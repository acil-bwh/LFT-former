...
class ViT(nn.Module):
    def __init__(self, *, num_vectors, vec_dim, num_classes, dim, depth, heads, mlp_dim,
        ...
        # Project input patch vectors into transformer dimension
    ...
    def forward(self, x):
        ...
        # ---- FIX: Auto-adjust positional embedding length ----
        pos_emb = self.pos_embedding
       
        if n + 1 != pos_emb.shape[1]:
            # interpolate to correct number of patches
            cls_pos = pos_emb[:, :1]
            patch_pos = pos_emb[:, 1:]
            patch_pos = torch.nn.functional.interpolate(
                patch_pos.permute(0, 2, 1),
                size=n,
                mode='linear',
                align_corners=False
            ).permute(0, 2, 1)
